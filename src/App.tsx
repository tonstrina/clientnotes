...
import { Plus, User, FileText, Download, Edit2, Trash2, Calendar, Search, LogOut, AlertCircle, MoreVertical, X } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [showClientMenu, setShowClientMenu] = useState<number | null>(null);
  ...
  const updateClient = async (clientId: number, newName: string): Promise<void> => {
    ...
    try {
      ...
      const updatedClients = clients.map(client => 
        client.id === clientId ? { ...client, name: newName.trim() } : client
      );
      
      setClients(updatedClients);
      
      if (selectedClient?.id === clientId) {
        setSelectedClient({ ...selectedClient, name: newName.trim() });
      }
  ...
  const deleteClient = async (client: Client): Promise<void> => {
    ...
    setSyncing(true);
    try {
      // First delete all notes for this client
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-purple-200">Fetching your notes...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-6">
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl p-4 sm:p-8 mb-8">
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-6">
            <div className="flex-1">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 mb-4">
                <div className="relative flex-shrink-0">
                  ...
                  <div className="relative bg-gradient-to-r from-blue-500 to-purple-600 p-3 sm:p-4 rounded-2xl">
                    <FileText className="h-8 w-8 sm:h-10 sm:w-10 text-white" />
                  </div>
                </div>
                <div className="min-w-0">
                  <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black bg-gradient-to-r from-white via-purple-200 to-blue-200 bg-clip-text text-transparent mb-2 sm:mb-3">
                    Client Notes
                  </h1>
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    <div className="flex items-center gap-2 px-3 py-2 bg-emerald-500/20 border border-emerald-400/30 rounded-full backdrop-blur-sm">
                      <div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div>
                      <span className="text-emerald-200 text-xs sm:text-sm font-semibold truncate">
                        {user.email}
                      </span>
                    </div>
                    {syncing && (
                      <div className="flex items-center gap-2 px-3 py-2 bg-blue-500/20 border border-blue-400/30 rounded-full backdrop-blur-sm">
                        <div className="w-3 h-3 sm:w-4 sm:h-4 border-2 border-blue-300 border-t-transparent rounded-full animate-spin"></div>
                        <span className="text-blue-200 text-xs sm:text-sm font-semibold">Syncing...</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <p className="text-base sm:text-lg text-purple-100 leading-relaxed">
                Your notes are securely stored in the cloud with enterprise-grade encryption
              </p>
            </div>
            ...
            <div className="hidden lg:flex gap-4">
              <button
                ...
                className="group relative px-6 py-4 bg-gradient-to-r from-red-600 to-red-700 rounded-2xl overflow-hidden transition-all duration-300"
              >
            ...
            {/* Mobile Buttons */}
            <div className="flex flex-col sm:flex-row gap-3 lg:hidden w-full">
              <button
                onClick={() => setShowAddClient(true)}
                disabled={syncing}
                className="group relative px-4 py-3 bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex-1 sm:flex-none"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-blue-700 to-blue-800 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
              ...
              {clients.length > 0 && (
                <button
                  onClick={exportAllNotes}
                  className="group relative px-4 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-2xl overflow-hidden transition-all duration-300 flex-1 sm:flex-none"
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-emerald-700 to-emerald-800 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                  <div className="relative flex items-center justify-center gap-2 text-white font-semibold">
              ...
              <button
                onClick={handleSignOut}
                className="group relative px-4 py-3 bg-gradient-to-r from-red-600 to-red-700 rounded-2xl overflow-hidden transition-all duration-300 flex-1 sm:flex-none"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-red-700 to-red-800 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                <div className="relative flex items-center justify-center gap-2 text-white font-semibold">
                          ...
                          {editingClient === client.id ? (
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter') {
                                    updateClient(client.id, editClientName);
                                  } else if (e.key === 'Escape') {
                                    setEditingClient(null);
                                    setEditClientName('');
                                  }
                                }}
                                className="flex-1 px-3 py-2 bg-white/20 border border-white/30 rounded-lg text-white placeholder-purple-300 text-base focus:ring-2 focus:ring-purple-400 focus:border-transparent"
                                autoFocus
                              />
                              <button
                                onClick={() => updateClient(client.id, editClientName)}
                                disabled={syncing || !editClientName.trim()}
                                className="p-2 text-green-300 hover:text-green-200 hover:bg-green-500/20 rounded-lg transition-all duration-200 disabled:opacity-50"
                              >
                              ...
                              <button
                                ...
                                className="p-2 text-purple-300 hover:text-purple-200 hover:bg-white/20 rounded-lg transition-all duration-200"
                              >
                                <X className="h-4 w-4" />
                              </button>
                            </div>
                            ...
                            <>
                              <div className="font-bold text-white text-lg truncate">{client.name}</div>
                              <div className="text-sm text-purple-200 flex items-center gap-4 mt-1">
                                <span className="flex items-center gap-2">
                                  <FileText className="h-4 w-4" />
                                  {client.notes?.length || 0} notes
                                </span>
                                <span className="hidden sm:inline">Added: {new Date(client.created_at).toLocaleDateString()}</span>
                              </div>
                            </>
                          )}
                        </div>
                      ...
                      {editingClient !== client.id && (
                        <div className="relative">
                          <button
                            ...
                            className="p-2 text-purple-300 hover:text-white hover:bg-white/20 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100"
                          >
                            <MoreVertical className="h-4 w-4" />
                          </button>
                          
                          {showClientMenu === client.id && (
                            <div className="absolute right-0 top-full mt-2 w-48 bg-white/20 backdrop-blur-xl border border-white/30 rounded-2xl shadow-2xl p-2 z-10">
                              <button
                                onClick={() => {
                                ...
                                disabled={syncing}
                                className="w-full flex items-center gap-3 px-4 py-3 text-purple-200 hover:text-white hover:bg-white/20 rounded-xl transition-all duration-200 disabled:opacity-50"
                              >
                              ...
                              <button
                                ...
                                disabled={syncing}
                                className="w-full flex items-center gap-3 px-4 py-3 text-red-300 hover:text-red-200 hover:bg-red-500/20 rounded-xl transition-all duration-200 disabled:opacity-50"
                              >
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white/20 backdrop-blur-xl border border-white/30 rounded-3xl shadow-2xl p-8 w-full max-w-md">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-white">Delete Client</h3>
                  <p className="text-purple-200">This action cannot be undone</p>
                </div>
              </div>
              
              <div className="mb-6">
                <p className="text-white mb-2">
                  Are you sure you want to delete <span className="font-bold text-blue-300">"{deletingClient.name}"</span>?
                </p>
                <p className="text-purple-300 text-sm">
                  This will permanently delete the client and all {deletingClient.notes?.length || 0} associated notes.
                </p>
              </div>
              ...
              <div className="flex gap-4 justify-end">
                ...
                <button
                  onClick={() => deleteClient(deletingClient)}
                  disabled={syncing}
                  className="px-8 py-3 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-2xl hover:from-red-700 hover:to-red-800 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed font-semibold"
                >
                  {syncing ? 'Deleting...' : 'Delete Client'}
                </button>
              </div>
            </div>
          </div>
